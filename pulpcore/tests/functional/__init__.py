...
def _otel_collector(status_api_client):
    ...
    # Not sure what the right time to wait is here but the server does not seem
    # to be found immediately.
    for _ in range(30):
        if unexpected_spans:
            # Reset unexpected spans
            unexpected_spans = []
            break
        # Tickle the telemetry stack
        status_api_client.status_read()
        time.sleep(1)
    else:
        pytest.fail("Telemetry is not responding.")
    ...
    if expected_spans:
        raise Exception(
            ...
            f"Unexpeced spans received: {unexpected_spans}"
        )


@pytest.fixture
def expect_otel_span(_otel_collector):
